...
namespace SeleniumAdvanced_Homework.Tests
{
    public class DynamicControlsTest : BaseTest
    {
        public void DynamicControls()
        {
            ...
            // Ищу кнопку Enabled
            var buttonEnable = WaitsHelper.WaitForVisibilityLocatedBy(By.CssSelector("button[onclick='swapCheckbox()']"));
            ...
            Assert.That(WaitsHelper.WaitForVisibilityLocatedBy(By.XPath("//*[contains(text(),'gone!')]")).Text, Is.EqualTo("It's gone!"));

            // Проверяю, что чекбокса нет
            Assert.That(WaitsHelper.WaitForElementInvisible(checkbox));

            // Проверяю, что поле ввода disabled
            Assert.That(WaitsHelper.WaitForElementInvisible(inputField));

            // Кликаю по кнопке Enabled
            buttonEnable.Click();

            // Проверяю, что элемент "It's enabled!" отобразилсяи и корректен
            Assert.That(WaitsHelper.WaitForVisibilityLocatedBy(By.XPath("//*[contains(text(),'enabled!')]")).Text, Is.EqualTo("It's enabled!"));
